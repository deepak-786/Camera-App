...
capBtn.addEventListener("click", function () {
    ...
    //origin shift(next 4 line in order to get zoomed effect)
...
navigator.mediaDevices
    ...
    .then(function (mediaStream) {
        mediaRecorder = new MediaRecorder(mediaStream);   //MediaRecorder is objectblueprint(create object)

        mediaRecorder.addEventListener("dataavailable", function (e) {
            chunks.push(e.data);  //to collect data into array
        });

        mediaRecorder.addEventListener("stop", function (e) {
            //for video we will save this object in db
            let blob = new Blob(chunks, { type: "video/mp4" });   //blob is an object that denotes a large file(combine chunks)
            chunks = [];
            addMedia(blob, "video");
            // let a = document.createElement("a");  //anchor tag
            // let url = window.URL.createObjectURL(blob);   //to create url of blob(browser defined function)
            // a.href = url;
            // a.download = "video.mp4";  //to download & name file
            // a.click();
            // a.remove();
        });
